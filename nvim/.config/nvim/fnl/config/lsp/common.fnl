...
(defn- update-colorscheme []
  ...
  ;; Useful for the HoldCursor and HoldCursorI
...
(defn- register-keybindings
  [bufnr]
  (which-key.register {:gq ["<Cmd>lua vim.lsp.buf.range_formatting()<CR><Esc>"
                            "Format selection"]}
                      {:buffer bufnr :mode :v})
  ...
  (which-key.register {:x {:name :transform
                           :x ["<Cmd>lua require('telescope.builtin').lsp_code_actions()<CR><Esc>"
                               "Code action"]}}
